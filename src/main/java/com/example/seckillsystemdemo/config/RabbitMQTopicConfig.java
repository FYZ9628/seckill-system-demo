...
/**
 * 实际项目中经常用到，使用也是最频繁的一个模式
 *
 * RabbitMQ配置类-Topic 主题交换机，是直连模式的拓展，所以使用的也是最多的
 * 两个符号的意义： # 号可以代表 0个或者多个 单词，例如  #.topic.#，可以是  topic.two.three，也可以是 one.two.topic.three
 *               * 号可以代表一个单词，例如， *.topic.*.*，只能是 one.topic.two.three，其他的都不行
 *
 * @author: LC
